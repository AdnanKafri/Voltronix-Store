@component('emails.layouts.notification', ['subjectLine' => __('emails.order_cancelled.subject', ['order_number' => $order->order_number])])
    <h2 style="margin:0 0 12px;">{{ __('emails.order_cancelled.title') }}</h2>
    <p style="margin:0 0 8px;">{{ __('emails.common.greeting', ['name' => $order->customer_name]) }}</p>
    <p style="margin:0 0 14px;">{{ __('emails.order_cancelled.message', ['order_number' => $order->order_number]) }}</p>
