...
@section('title', __('app.auth.login_title') . ' - Voltronix')
...
.voltronix-form-check input[type="checkbox"] {
    width: 18px;
    height: 18px;
    accent-color: #007fff;
}

.voltronix-form-check label {
    color: rgba(255, 255, 255, 0.8);
    font-size: 0.9rem;
    cursor: pointer;
}
...
.voltronix-alert.success {
    border-color: rgba(40, 167, 69, 0.4);
    background: rgba(40, 167, 69, 0.1);
}

.voltronix-alert.info {
...
<div class="voltronix-auth-container">
    <div class="voltronix-auth-card">
        <div class="voltronix-auth-header">
            <h1 class="voltronix-auth-title">{{ __('app.auth.login_title') }}</h1>
            <p class="voltronix-auth-subtitle">{{ __('app.auth.login_subtitle') }}</p>
        </div>
        
        <!-- Session Status -->
        @if (session('status'))
            <div class="voltronix-alert success">
        ...
        @if (session('message'))
            <div class="voltronix-alert info">
                <i class="bi bi-info-circle {{ app()->getLocale() == 'ar' ? 'ms-2' : 'me-2' }}"></i>
                {{ session('message') }}
            </div>
        @endif

        <form method="POST" action="{{ route('login') }}" id="voltronixLoginForm">
            @csrf
            ...
            <!-- Remember Me -->
            <div class="voltronix-form-check">
                <input type="checkbox" id="remember_me" name="remember">
                <label for="remember_me">{{ __('app.auth.remember_me') }}</label>
            </div>

            <!-- Submit Button -->
            <button type="submit" class="voltronix-submit-btn" id="submitBtn">
                <i class="bi bi-box-arrow-in-right {{ app()->getLocale() == 'ar' ? 'ms-2' : 'me-2' }}"></i>
            ...
            <div class="voltronix-auth-links">
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="voltronix-auth-link">
                        {{ __('app.auth.forgot_password') }}
                    </a>
                    <br><br>
                @endif
                <span class="voltronix-auth-text">{{ __('app.auth.not_registered') }}</span>
                <a href="{{ route('register') }}" class="voltronix-auth-link">
                    {{ __('app.nav.register') }}
                </a>
            </div>
        </form>
...
@push('scripts')
<script>
document.getElementById('voltronixLoginForm').addEventListener('submit', function(e) {
